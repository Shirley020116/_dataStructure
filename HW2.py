...
# HW2 Labels for Cryptocurrency Market Sentiment Classification
ITEMS = ["Market Optimism", "Panic Selling", "Hype", "Uncertainty", "Regulation Fear"]
...
def process_batch_dialogue(client, dialogues: list, delimiter="-----"):
    ...
    """
    prompt = (
    "你是一位虛擬貨幣市場分析專家，請根據以下指標判斷每則新聞或社群貼文的情勢狀態：\n"
    + "\n".join(ITEMS) +
    "\n\n請依據情緒內容，對每個指標：若有明顯出現請標記為 1，否則留空。"
    " 請對每段內容產生 JSON 格式回覆，並在各段間用下列分隔線隔開：\n"
    f"{delimiter}\n"
    "例如：\n"
    "```json\n"
    "{\n  \"Fear\": \"1\",\n  \"Hype\": \"1\",\n  \"Uncertainty\": \"\" }\n"
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python HW2.py <path_to_csv>")
        sys.exit(1)
